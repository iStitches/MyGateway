...
import org.xjx.gateway.starter.Singleton;
import org.xjx.gateway.starter.cache.config.RedisDistributedProperties;
import org.xjx.gateway.starter.cache.core.CacheGetFilter;
import org.xjx.gateway.starter.cache.core.CacheGetIfAbsent;
import org.xjx.gateway.starter.cache.core.CacheLoader;
import org.xjx.gateway.starter.cache.toolkit.CacheUtil;
import org.xjx.gateway.starter.cache.toolkit.FastJson2Util;
...
public class StringRedisTemplateProxy implements DistributedCache {
    ...
    public void test() {
        RScoredSortedSet<Object> scoreSets = redissonClient.getScoredSortedSet("aaa");
        long currentTimestamp = System.currentTimeMillis();
        scoreSets.removeRangeByScore(0, true, currentTimestamp, false);
    }
    ...
    public <T> T safeGet(String key, Class<T> clazz, CacheLoader<T> cacheLoader, long timeout, TimeUnit timeUnit,
                         RBloomFilter<String> bloomFilter, CacheGetFilter<String> cacheGetFilter, CacheGetIfAbsent<String> cacheGetIfAbsent) {
        T result = get(key, clazz);
        // 缓存结果不等于空或空字符串直接返回；通过函数判断是否返回空，为了适配布隆过滤器无法删除的场景；两者都不成立，判断布隆过滤器是否存在，布隆过滤器不存在就说明该值一定不存在，直接返回
        if (!CacheUtil.isNullOrBlank(result)
